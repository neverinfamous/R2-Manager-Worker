...
async function listWebhooks(
    ...
    try {
        ...
        await logError(env, error instanceof Error ? error : String(error), { module: 'webhooks', operation: 'list_webhooks' }, isLocalDev);
...
async function getWebhook(
    ...
    try {
        ...
        await logError(env, error instanceof Error ? error : String(error), { module: 'webhooks', operation: 'get_webhook', metadata: { webhookId } }, isLocalDev);
        return errorResponse(
...
async function createWebhook(
    ...
    if (!body?.name || !body?.url || (body?.events?.length ?? 0) === 0) {
        return errorResponse('Missing required fields: name, url, events', corsHeaders, 400);
    }
    ...
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?)`
        ...
        const webhook = await env.METADATA.prepare(
            'SELECT * FROM webhooks WHERE id = ?'
        ).bind(id).first<Webhook>();

        return jsonResponse({ webhook }, corsHeaders, 201);
    } catch (error) {
        await logError(env, error instanceof Error ? error : String(error), { module: 'webhooks', operation: 'create_webhook' }, isLocalDev);
        return errorResponse(
...
async function updateWebhook(
    ...
    if (isLocalDev) {
        ...
        const updated: Webhook = {
            ...
            enabled: body.enabled !== undefined ? (body.enabled ? 1 : 0) : webhook.enabled,
            updated_at: nowISO(),
        };
    ...
    try {
        ...
        await logError(env, error instanceof Error ? error : String(error), { module: 'webhooks', operation: 'update_webhook', metadata: { webhookId } }, isLocalDev);
        return errorResponse(
...
async function deleteWebhook(
    ...
    try {
        ...
        await logError(env, error instanceof Error ? error : String(error), { module: 'webhooks', operation: 'delete_webhook', metadata: { webhookId } }, isLocalDev);
        return errorResponse(
...
async function testWebhook(
    ...
    try {
        ...
        await logError(env, error instanceof Error ? error : String(error), { module: 'webhooks', operation: 'test_webhook', metadata: { webhookId } }, isLocalDev);
        const result: WebhookTestResult = {
            success: false,
