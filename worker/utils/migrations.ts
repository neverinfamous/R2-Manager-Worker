...
export const MIGRATIONS: Migration[] = [
    {
        version: 1,
        name: 'initial_schema',
        description: 'Base schema with bulk_jobs, job_audit_events, audit_log tables',
        sql: `
    ...
    },
    {
        version: 3,
        name: 'bucket_tags',
        description: 'Add bucket_tags table for custom text-based bucket organization',
        sql: `
...
export async function detectLegacyInstallation(db: D1Database): Promise<LegacyInstallationInfo> {
    try {
        ...
        if (existingTables.includes('bulk_jobs') && existingTables.includes('audit_log')) {
            suggestedVersion = 1;
        }
