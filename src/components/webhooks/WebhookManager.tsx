...
const CheckCircleIcon = ({ className }: { className?: string }): ReactElement => (
    <svg className={className} xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="webhook-manager-container">
            ...
            <div className="webhook-header">
                ...
                <div className="webhook-header-actions">
                    ...
                    <button className="webhook-btn webhook-btn-primary" onClick={openCreateDialog}>
                        <PlusIcon />
                        Add Webhook
                    </button>
                </div>
            ...
            {!loading && webhooks.length > 0 && (
                <div className="webhook-list">
                    {webhooks.map((webhook) => {
                        ...
                        return (
                            <div key={webhook.id} className={`webhook-card ${webhook.enabled === 0 ? 'disabled' : ''}`}>
                                <div className="webhook-card-header">
                                    <div className="webhook-card-title-area">
                                        ...
                                        <div className="webhook-card-info">
                                            <h4>
                                                {webhook.name}
                                                {webhook.secret && (
                                                    <span className="webhook-secure-badge" title="HMAC signature enabled">
                                                        <ShieldIcon />
                                                    </span>
                                                )}
                                            </h4>
                                    ...
                                    <span className={`webhook-status-badge ${webhook.enabled === 1 ? 'enabled' : 'disabled'}`}>
                                        {webhook.enabled === 1 ? 'Enabled' : 'Disabled'}
                                    </span>
                                </div>
                                <div className="webhook-events">
                                    {events.map((event) => (
                                        <span key={event} className="webhook-event-tag" title={WEBHOOK_EVENT_DESCRIPTIONS[event]}>
                                            {WEBHOOK_EVENT_LABELS[event]}
                                        </span>
                                    ))}
                                </div>
                                <div className="webhook-card-footer">
                                    <span className="webhook-card-meta">Updated: {formatDate(webhook.updated_at)}</span>
                                    <div className="webhook-card-actions">
                                        ...
                                        >
                                            {testingWebhook === webhook.id ? <LoaderIcon /> : <PlayIcon />}
                                            Test
                                        </button>
                                        <button
            ...
            {(showCreateDialog || editingWebhook !== null) && (
                <div className="webhook-dialog-overlay" onClick={(e) => {
                    ...
                    <div className="webhook-dialog">
                        ...
                        <div className="webhook-dialog-body">
                            <div className="webhook-form-group">
                                ...
                                />
                                <span className="webhook-form-hint">A descriptive name for this webhook</span>
                            </div>
                            <div className="webhook-form-group">
                                ...
                                />
                                <span className="webhook-form-hint">The endpoint that will receive webhook POST requests</span>
                            </div>
                            <div className="webhook-form-group">
                                ...
                                />
                                <span className="webhook-form-hint">If set, requests will include an X-Webhook-Signature header</span>
                            </div>
                            <div className="webhook-form-group">
                                <label>Events</label>
                                <div className="webhook-events-grid">
                                    {ALL_WEBHOOK_EVENTS.map((event) => (
                                        <div key={event} className="webhook-checkbox-wrapper">
                                            ...
                                            />
                                            <label htmlFor={`event-${event}`} className="webhook-checkbox-label">
                                                {WEBHOOK_EVENT_LABELS[event]}
                                            </label>
                                        </div>
                                    ))}
                            ...
                            <div className="webhook-checkbox-wrapper">
                                ...
                                />
                                <label htmlFor="webhook-enabled" className="webhook-checkbox-label">
                                    Enabled
                                </label>
                            </div>
                        </div>
                ...
                <div className="webhook-dialog-overlay" onClick={(e) => {
                    ...
                    <div className="webhook-dialog">
                        <div className="webhook-dialog-header">
                            ...
                            <p>Are you sure you want to delete "{deletingWebhook.name}"? This action cannot be undone.</p>
                        </div>
                        <div className="webhook-dialog-actions">
                            <button
                                className="webhook-btn webhook-btn-secondary"
