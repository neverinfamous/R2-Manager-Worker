...
export function AISearchQuery({ instanceName, onBack }: AISearchQueryProps): JSX.Element {
      ...
      <form className="ai-search-form" onSubmit={handleSearch}>
        ...
        <div className="ai-search-input-row">
          <input
            ...
            placeholder={searchMode === 'ai' ? 'Ask a question about your data...' : 'Search for relevant content...'}
              ...
              <div key={result.file_id} className="ai-search-result-card">
                ...
                {result.attributes?.folder && (
                  <div className="ai-search-result-meta">
                    <span className="result-folder">📁 {result.attributes.folder}</span>
                  </div>
                )}
              </div>
            ))}
