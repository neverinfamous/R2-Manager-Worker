...
export function calculateHealthScore(health: HealthSummary): {
    ...
    if (health.buckets.total > 0) {
        const colorCoveragePercent = (health.buckets.withColors / health.buckets.total) * 100;
    ...
    if (health.buckets.total > 0) {
        const tagCoveragePercent = (health.buckets.withTags / health.buckets.total) * 100;
